      ...
      switchMap(({ page, limit, sort, order }) =>
        this.orderService.getMyOrders(page, limit, sort, order).pipe(
          ...
          map(({ ordersList, count }) => {
            this.router.navigateByUrl(`/orders?page=${page + 1}&limit=${limit}&sort=${sort}&order=${order}`);
            return orderActions.loadOrdersSuccess({ ordersList, count });
          }),
          catchError((err: IErrorResponse) => [orderActions.loadOrdersFailure({ message: err.error.message })])
        )
      )
    )
  );
    ...
    this.actions$.pipe(
      ...
      switchMap(({ orderId }) =>
        this.orderService.getMyOrder(orderId).pipe(
          ...
          catchError((err: IErrorResponse) => [orderActions.loadOrderFailure({ message: err.error.message })])
        )
      )
    )
  );
    ...
    this.actions$.pipe(
      ...
      switchMap(({ productId, rate, comment }) =>
        this.orderService.rateProduct(productId, rate, comment).pipe(
          takeUntil(this.actions$.pipe(ofType(orderActions.rateOrdererProductCancel))),
          map(({ rating }) => {
            this.notificationService.showMessage("Thank you for your review.", "success");
    ...
    this.actions$.pipe(
      ...
      switchMap(({ orderId, _id }) =>
        this.orderService.getCurrentProduct(orderId, _id).pipe(
          takeUntil(this.actions$.pipe(ofType(orderActions.loadOrderProductCancel))),
          map(({ product }) => {
